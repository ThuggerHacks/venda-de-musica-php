...
<body>
    ...
    <!---------------dvds-->
     ...
     <section id="dvd">
        <h3>ARQUIVOS DE MIDIA</h3>
        <?php
             $sql = "SELECT * FROM projecto WHERE tipo  NOT IN(?,?,?,?)";
            $motor = $con->prepare($sql);
            $motor->execute([
                "Musica","Album","Mixtape","Video"
            ]);
        ...
        <div class="card-header">
            ...
            <div class="card-body">
                ...
                <strong>Faixas/Videos: </strong> <span><?=$linha['num_faixa']?></span><br>
                <strong>Dura&ccedil;&atilde;o: </strong> <span><?=$linha['duracao']?> Minuto(s)</span><br>
                <strong>Capacidade: </strong> <span><?=$linha['lados_dvd']?></span><br>
